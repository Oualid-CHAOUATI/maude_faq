...
try{
  ...
  $reqString="SELECT n.num,n.libelle as nat_libelle,c.libelle as cont_libelle from (nationalite n inner join continent c on n.numContinent = c.num )";

  $where = false;
  if(trim($_GET["search-value"])!=""){
    $searchValue = $_GET["search-value"];
    
    $reqString .= "where LOWER(n.libelle) like LOWER('%$searchValue%')";
    $where=true;

  }
  if(trim($_GET["continent"])!=""){
    $numContinent = $_GET["continent"];
    if(!$where)$reqString .= "where ";
    else $reqString.=" and";
    $reqString .= " c.num=$numContinent";

  }


  $req=$pdo->prepare($reqString." ORDER BY n.num");
...
<form class="mx-auto">   
    ...
    <div class="relative">
        ...
        <input type="search" id="default-search" class="block w-full p-4 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search ..."  name="search-value" value="<?=$searchValue?>" >
        <button type="submit" class="text-white absolute right-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" >Search</button>
    </div>

    <select id="select" class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"   name="continent">
    <option value=""selected > choisir un continent</option>  

  <?php foreach($continents as $continent):?>
    <option value="<?= $continent['num'] ?>" <?php if($continent['num']==$numContinent):?> selected <?php endif ?>> <?=$continent["libelle"]?></option>  
    <?php endforeach?>
  </select>
  </form>
